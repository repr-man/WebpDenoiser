...
class ModulatedDeformableConv2d(nn.Module):
    ...
    def init_offset(self):
        init.constant_(self.conv_offset_mask.weight, 0.)
        init.constant_(self.conv_offset_mask.bias, 0.)
    ...
    def forward(self, x):
        ...
        #torch.cond(offset_mean > max(x.shape[2:]),
        #           lambda: logger.warning('Offset mean is {}, larger than max(h, w).'.format(offset_mean)),
        #           lambda: None
        #           )
