...
from graph import FinalNode, MaskNode, PngNode, RCANComputationNode, ErrorNode, WebpNode, Conv1Node
...
@flaskServer.route("/get-image-rcan", methods=["POST"])
def requestGetRcan():
    """
    Receives a JSON payload of the form:
    {
    ...
    Returns the contents of the file at
    '/path/to/root/dir/rcan/filename.png'.
    """
    root, fileName = getProjectParts(request)
    return RCANComputationNode().getBytes(root, fileName)
